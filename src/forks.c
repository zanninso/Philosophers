...
void set_fork_ids(unsigned int philo_id, t_env *env)
{
    char tab[2] = {1,-1};
    unsigned int tmp = philo_id + env->philos_number;
    env->philos[philo_id].right_fork = (tmp + tab[!philo_id%2]) % env->philos_number;
    env->philos[philo_id].left_fork = (tmp + tab[philo_id%2]) % env->philos_number;
}
...
{
    (void)val;
    pthread_mutex_unlock(&env->forks[env->philos[philo_id].left_fork].lock);
    pthread_mutex_unlock(&env->forks[env->philos[philo_id].right_fork].lock); 
}
...
inline void get_forks(unsigned int philo_id, t_env *env)
{
    pthread_mutex_lock(&env->forks[env->philos[philo_id].left_fork].lock);
    pthread_mutex_lock(&env->forks[env->philos[philo_id].right_fork].lock);
}